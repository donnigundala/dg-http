...
func setupCorsRouter(mw gin.HandlerFunc) *gin.Engine {
...
func TestCORS_AllowedOrigins(t *testing.T) {
	...
	r := setupCorsRouter(middleware.CORS(config))
...
func TestCORS_PreflightRequest(t *testing.T) {
	...
	// Since we are using Gin, and the middleware handles OPTIONS, status should likely be 204 or dictated by middleware
...
func TestCORSWithDefault(t *testing.T) {
	r := setupCorsRouter(middleware.CORSWithDefault())
	...
	// Default config (usually * or similar) check
